export default function HowItWorks() {
  ...
  return (
    <section className="w-full max-w-6xl lg:max-w-5xl mx-auto px-4 py-24 relative z-20 select-none">
      <div className="text-center mb-16">
        <h2 className="text-3xl md:text-4xl lg:text-4xl font-extrabold text-gray-900 mb-4 tracking-tight">The Data Pipeline</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 lg:gap-10 relative z-10">
          {steps.map((step, idx) => (
            <div key={step.title} className="group relative animate-[hover-float_4s_ease-in-out_infinite] will-change-transform" style={{ animationDelay: `${idx * 0.4}s` }}>
              ...
              <div className="relative h-full pt-10 lg:pt-12 pb-8 lg:pb-8 px-6 sm:px-8 bg-white/30 backdrop-blur-xl rounded-3xl border border-white/60 shadow-[0_20px_60px_rgba(0,0,0,0.05),inset_0_0_30px_rgba(255,255,255,0.6)] flex flex-col items-center text-center group-hover:-translate-y-2 group-hover:shadow-[0_30px_60px_rgba(99,102,241,0.15),inset_0_0_40px_rgba(255,255,255,0.8)] transition-all duration-500 z-10 will-change-transform">
                <div className="text-indigo-800 font-mono text-xs sm:text-sm font-extrabold mb-4 tracking-widest uppercase bg-indigo-100/50 inline-block px-3 py-1 rounded-full w-max border border-indigo-200">Stage 0{idx + 1}</div>
                <h3 className="text-2xl font-black text-gray-900 mb-3">{step.title}</h3>
                <p className="text-gray-800 text-sm sm:text-base font-semibold leading-relaxed">{step.desc}</p>
              </div>
            </div>
          ))}
        </div>
